...
| **Setup Default Option** | `Toggle` | **Mark as the default disposition option** for quick selection | Enabled  / Disabled  |
| **Call Status** | `Enum` | Current status of the disposition | `Completed`, `Answered`, `No`, `Ringing`, `Busy`, `Cancelled`, `In Progress`, `Failed`, `AMD` |
| **Color Preview** | `Swatch` | Visual color indicator (text + background) for the disposition<br/>Useful for quick recognition and workflow mapping | |
...
#### ⚙️ Setup Default Option
- **Function**: Marks this disposition as the default option for quick selection
- **Use Cases**: Enable for your most frequently used disposition (e.g., "Completed", "No Answer")
- **Impact**: Speeds up disposition assignment during high-volume calling
...
| **Assign role View** | Assign which user roles can see/apply this disposition.<br/>Select one or more from the available roles. | (e.g. Admin, Agent)                                              |
...
| **Fields For Notes** | Optional. Select additional note fields, typically available after setting a location.<br/>*Requires GHL Location to be selected first.* | Unique to each CRM location                                      |
| **Text Color** / **Select Text Color** | Choose the text color for this disposition label in the interface.                                     | Example: White (`#FFFFFF`)                                       |
| **Background Color** / **Select Background color** | Select a background color for the disposition button.<br/>Aids in quick visual identification in call logs and workflow mapping. | Example: Blue (`#0000FF`), Green (`#00FF00`)                     |

> The form includes all the above fields in a two-column layout. **Assign role View** allows you to select multiple roles. **Fields For Notes** dynamically appears once a GHL Location is selected (shows "Select a location first" until then). Use the text and background color pickers to customize appearance for clarity in dashboards and call screens.
...
> - **Assign role View:** Multi-role selection (e.g., Agent, dev)
> - **GHL Location:** Select relevant CRM/account
> - **Fields For Notes:** Appears after location is selected
> - **Text Color & Background Color** / **Select Text Color & Select Background color:** Color pickers for customizing the label/button
