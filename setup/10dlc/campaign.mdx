...
<img
  ...
  alt="10DLC Campaign Dashboard listing campaigns and their approval/status details"
  className="rounded-lg border shadow-sm"
/>

### 📊 Campaign Dashboard Table

The campaign dashboard displays all your existing 10DLC campaigns in a filterable table, letting you quickly view status and manage each one. The columns in this table are:

| Column                | Description                                                         | Example/Values                  |
|-----------------------|---------------------------------------------------------------------|---------------------------------|
| **Campaign Alias**    | The internal name you assign to the campaign for easy tracking      | Onboarding Campaign             |
| **Provider**          | Messaging provider handling the campaign's SMS traffic              | Ai Sync, Twilio                   |
| **Brand ID**          | The registered brand this campaign is linked to                     | 245127                          |
| **Vertical**          | Industry/category for compliance                                    | EDUCATION, RETAIL, HEALTHCARE   |
| **Use Case**          | The intended messaging use (notifications, marketing, customer care)| Notification, Support           |
| **Registration Status** | Carrier approval status for the campaign                          | APPROVED, PENDING, REJECTED     |                     |
| **Action**            | Manage each campaign (edit, delete, view details)                   | Edit/Delete                     |

<Info>
You can filter, sort, or search campaigns using the dashboard controls. If no campaigns are shown, click **"Add New Campaign"** to get started!
</Info>
...
- **Provider**  
  *Pre-set by your account (e.g., "Ai Sync"). Ensures the right SMS carrier is used.*
...
#### ⚙️ Optional Features (Enable as appropriate)
- **Embedded Phone**: Lets you include clickable phone numbers within SMS messages.
- **Age Gated**: Must be enabled for campaigns related to age-restricted (18+) content.
- **Affiliate Marketing**: Enable if the campaign content contains affiliate or referral promotions.
- **Direct Lending**: Required for campaigns involving credit, lending, or related financial products.
- **Embedded Link**: Allows SMS messages to contain clickable web links.
