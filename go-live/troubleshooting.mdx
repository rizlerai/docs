...
<Accordion title="'The dashboard shows zero but we're dialling'">
**Cause:** almost always the time zone on the profile, or a stale page.

**Fix:** hard refresh first (**Cmd+Shift+R** / **Ctrl+F5**). Then open the profile in the top right and confirm the time zone matches where the reps sit. If it is wrong, today's calls are filed under a different day — the data is there, under the wrong date.
</Accordion>

<Accordion title="'Our connect rate dropped but we're dialling the same'">
**Cause:** usually the numbers, not the reps.

**Fix:** open [Dialer Performance](/dashboard/dialer-performance) and look at the outcome bar. Rising **Declined** or **Screened** means your caller IDs are getting flagged. Rising **Bad number** means the list needs cleaning. See [Caller ID and number health](/dialer/caller-id-and-numbers).
</Accordion>

<Accordion title="'A rep shows a dash instead of a connect rate'">
**Cause:** working as designed. Under 10 dials there is not enough to rate.

**Fix:** nothing. A rate off four dials is noise, and showing it as a percentage is how a good rep gets judged on nothing. It appears as soon as they have the dials.
</Accordion>

<Accordion title="'Voicemails are being counted as connects'">
**Cause:** they are not. Voicemail, answering machines and screened calls can never be connects.

**Fix:** check which number they are reading. **Answered** includes voicemail; **Connects** does not. See [How calls actually work](/dialer/how-calls-work).
</Accordion>

<Accordion title="'The chart doesn't match the total above it'">
**Cause:** usually inbound versus outbound. **All calls** includes inbound; the outcome breakdown and the daily chart are outbound dials only.

**Fix:** the tile subtitle states how many were dialled out — that is the figure the panels below reconcile to. If the outcome segments genuinely do not add up to the dial count, that is a real bug. Report it.
</Accordion>

<Accordion title="'My AI agent gave me a different number than the dashboard'">
**Cause:** nearly always a different window or scope.

**Fix:** ask the agent *"what date range and which reps did that cover?"* and compare like for like. See [Asking your agent about your data](/agents/asking-about-your-data).
</Accordion>
...
<Accordion title="'The screening message never played on my phone'">
**Cause:** iPhone Screening off for that call, or no script selected.

**Fix:** confirm the **iPhone Screening** toggle is on and a **recorded** iPhone Script is selected — a typed script cannot play and the dialer blocks Start until an audio one is chosen, then test by dialling your own screened phone. See [iPhone and carrier screening](/dialer/iphone-screening).
</Accordion>

<Accordion title="'A screened call got marked as a machine'">
**Cause:** a screening prompt sounds like a recorded greeting, so systems judging audio rhythm alone mistake it for voicemail.

**Fix:** AI Sync reads the words rather than the cadence, so report specific calls with the time and the lead's number and they can be traced.
</Accordion>
...
<Accordion title="'It won't let me pick a number'">
...
**Fix:** read the wording. "They picked up" means a real person. "We could not tell if anyone picked up" means the system could not confirm one — still worth a call, but not a warm lead. "A voicemail answered" means a machine, and that one is not added to Callbacks at all.
...
**Cause:** the disposition was added after the account was created, so no rule links it to a stage. Names are not the mechanism, or the lead is in a pipeline with no appointment stage.

**Fix:** renaming will not help. Test a starter disposition first — if that moves a lead and the new one does not, this is the known limitation. See [Dispositions and pipeline mapping](/go-live/dispositions-and-pipeline). Remember AI Sync pipelines and GoHighLevel pipelines are separate — adding a stage in GoHighLevel does not create it here.
</Accordion>
