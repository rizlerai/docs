...
Let your agent know **when and how to end a call**—so it wraps up conversations at the right time.

1. Click **Configure** on the End Call card.
2. A panel appears with a single large **Description** box.
3. Enter simple step-by-step guidance in everyday language. Example: "After confirming the appointment is booked, thank the caller and end the call politely."
4. Click **Save** to apply, or **Cancel** to close without saving.
...
<img
  ...
  alt="Appointment booking options and calendar selection"
  className="rounded-lg border shadow-sm my-4"
/>

1. From the **Calendar Appointments** card, click **Configure**.
3. If you select **Direct Calendar Booking**:
...
4. **Appointment Title:** Set the event name shown on the calendar (e.g., “Sales Call”, “Consultation”).
5. **Booking Window:** Set how many **days into the future** your agent can offer appointments (e.g., show only the next 7 days).
6. **Collect Contact Information:** Choose the caller details your agent must collect before booking (options depend on calendar platform).
7. **Calendar Timezone:** Match the calendar’s timezone to your business/location needs.
8. Click **Save** to apply changes or **Cancel** to discard edits.

*Customizing appointment details, contact fields, and extra options:*

<img
  src="/images/ai/AI-agent/edit/advance/calendar-2.png"
  alt="Appointment details customization, contact collection, and configuration options"
  className="rounded-lg border shadow-sm my-4"
/>
...
**Purpose:** Choose **who receives notifications** about this agent — by email or SMS — and keep those destinations in one list.

<img
  src="/images/ai/AI-agent/edit/advance/sender-managment.png"
  alt="Sender Management table with Add Sender button"
  className="rounded-lg border shadow-sm my-4"
/>

The table lists every sender destination configured for this agent:

| Column | What it shows |
|--------|---------------|
| **ID** | Unique ID for the sender row |
| **Send To** | Destination address — an email or phone number |
| **Send Type** | How the notification is sent: **Email** or **SMS** |
| **Action** | Manage or remove that sender |

Use **Search** to find a destination quickly, and **Show entries** / pagination when the list grows.

### ➕ Add a sender

1. Click **+ Add Sender**.
2. Enter **Send To** — the email address or phone number that should receive notifications.
3. Choose **Send Type**:
   - **Email** — notifications go to that email address.
   - **SMS** — notifications go as a text message to that phone number.
4. Save to add the row to the table.

If no senders are configured yet, the table shows **No data available in table** until you add the first one.

Use the row **Action** controls to update or remove an existing sender when your notification destinations change.
