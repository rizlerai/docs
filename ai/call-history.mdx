...
## 📤 Exporting Your Call Data

When you export call history, the export file reflects exactly what you see in your filtered table—using all the same filters (like date, agent, call duration, direction, sentiment, success, phone numbers, and hangup reason).

Each exported row will include:

- A unique **ID** (per row, in order)
- **Time** (displayed in your time zone)
- **Agent Name**
...
- **Call Summary** (the complete summary, not just a snippet)
- **Call Transcript** (the full conversation, not just a preview)

> **Note:** Audio recordings are *not* included in exported files. To listen to call recordings, use the main list or the detail screen.
