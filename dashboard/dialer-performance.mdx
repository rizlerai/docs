...
<Update label="22 Sep 2026" description="Dashboard">
**This whole dashboard was rebuilt.** It now shows what happened to every single
dial in eleven categories that always add up to your dial count, so nothing is
hidden in an "other" pile.

**Three numbers were corrected.** Average talk time now counts every
conversation rather than only the long ones, so it reflects your real floor.
Connect rate now counts voicemail-answered dials in the total, because a dial
that reached a machine is still a dial. The daily chart now counts outbound
calls only, in your own time zone.

Your connect rate may read slightly lower than before. It was previously
overstated — nothing about your calling changed.

**The leaderboard moved here** from its own tab, with sorting and CSV export.
</Update>
